...
const UpdateIceCreamComponent = () => {
    ...
    const updateIceCream = (e) =>{

        e.preventDefault()
        const iceCream = {flavourCode, flavour, toppings, cost}

        IceCreamShopService.updateIceCream(id, iceCream).then((response) => {
  ...
  return (
    <Container>
        <Paper elevation = {3} style={paperStyle}>
            <h2 style={{color:'blue', textAlign:'center'}}>Update Ice Cream</h2>
            <Box
                component="form"
                sx={{
                    '& > :not(style)': { m: 1},
...
export default UpdateIceCreamComponent;